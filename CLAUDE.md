...
still appears in `/chapters` with its title, its difficulty badge and
"0 verses · Nothing here yet." Chapter 1 sat like that, labelled *Beginner*, at
the very top of the list — the first thing a browsing reader saw. Nothing
errored and no check caught it, because every check tested pages that had
content. `smoke-test.sh` now asserts that `/chapter/1` reports 8 verses. The
general lesson: the tracks hide unwritten chapters from the path, and `/chapters`
does not.
...
reference is silently missing from Research mode. `seed_ch16.sql` declared
ten and loaded nine before anybody counted. Until there is a tool for it, the
check is manual and takes one query: count the `UNION ALL` arms in the block,
then `SELECT COUNT(*)` for that chapter after loading. They have to match.
...
**A content-safety regex needs word boundaries or it cries wolf on its own
refusals.** The chapter 1 sweep for self-harm language flagged an example because
`hang` matched inside *changed*, and the sweep for named conditions flagged two
rows because `diagnos` matched *"worth reading rather than diagnosing"* and
*"the one they have just diagnosed in the other"* — both of which are the text
...
**Three chapters now say the same wellbeing thing in three vocabularies, and
they cross-reference each other on purpose.** 6.5 says a person can lift
themselves or let themselves sink; 13.29 says `na hinasty ātmanā ātmānam`, he
does not injure the self by the self, in the identical grammatical construction;
17.19 puts practice undertaken by hurting yourself in the bottom category by
name. A reader who lands on any one of them is offered the other two. When a
...
(ajñāna-ja, born of not-seeing) is put in front of the behaviour. Chapter 13 is
the hardest case of all, because there nobody has to be arguing in bad faith:
*you are not this, you are the one watching it* is a sentence a reader in pain
turns on themselves, and it feels like progress while it is doing damage. The
answer was in the chapter — 13.6 puts awareness inside the field, so there is no
watcher to climb into — and the practices in `seed_ch13.sql` are the only set in
the corpus written under an explicit ban: not one of them asks the reader to step
back from, disidentify from or observe themselves from a distance. 5.18 carries two at once, in opposite directions: the word must
...
person and not a chapter. 17.19 is the strongest of them all, because there the
refusal is the text's own: practice done by hurting yourself is named and put
in the bottom category, so the problem is the category and not the dose. `smoke-test.sh` asserts all of them by literal string. If one of
